...
Canvas {
	...
	property real length: 0.4 * width

	property real lineWidth: 2

	property real headLengthRatio: 0.25
	...
	property vector3d implicitXVector: Qt.vector3d(length, 0, 0)

	property vector3d implicitYVector: Qt.vector3d(0, length, 0)

	property vector3d implicitZVector: Qt.vector3d(0, 0, length)
	...
	function drawArrow(ctx, vector, color, label)
	{
		...
		var headLength = root.length * root.headLengthRatio

		ctx.lineWidth = lineWidth
		ctx.strokeStyle = Qt.darker(color, Math.max(1.0, 1.0 - vector.z / root.length))
		ctx.fillStyle = Qt.darker(color, Math.max(1.0, 1.0 - vector.z / root.length))
		ctx.beginPath()
		...
		var cosAngle = Math.cos(angle)
		var headVector = rotateVector2d(vector, angle).normalized().times(headLength / cosAngle)
		var offsetVector = normalizedProjectionVector.times(headLength * (1 - projectionRatio))
		ctx.lineTo(center + vector.x - headVector.x + offsetVector.x, center - vector.y + headVector.y - offsetVector.y)
		headVector = rotateVector2d(vector, -angle).normalized().times(headLength / cosAngle)
		ctx.lineTo(center + vector.x - headVector.x + offsetVector.x, center - vector.y + headVector.y - offsetVector.y)
		ctx.closePath()
		ctx.fill()

		ctx.fillStyle = Qt.darker(labelColor, Math.max(1.0, 1.0 - vector.z / root.length))
		ctx.textAlign = "center"
		ctx.textBaseline = "middle"
		ctx.font = headLength.toString() + "px monospace"
		offsetVector = normalizedProjectionVector.times(0.5 * headLength * projectionRatio)
